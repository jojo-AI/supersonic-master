...
public class HttpHeaderUserStrategy implements UserStrategy {
    ...
    private User getUser(Claims claims) {
        ...
        String tenantId = String.valueOf(claims.get(UserConstants.TOKEN_TENANT_ID));
        String apiKey = String.valueOf(claims.get(UserConstants.TOKEN_API_KEY));
        String datasetKey = String.valueOf(claims.get(UserConstants.TOKEN_DATASET_KEY));
        return User.get(userId, userName, displayName, email, isAdmin, tenantId, apiKey, datasetKey);
    }
