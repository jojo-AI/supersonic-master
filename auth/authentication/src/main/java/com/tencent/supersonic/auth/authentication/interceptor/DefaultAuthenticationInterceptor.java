...
public class DefaultAuthenticationInterceptor extends AuthenticationInterceptor {
    ...
    public UserWithPassword getUserWithPassword(HttpServletRequest request) {
        ...
        String tenantId = String.valueOf(claims.get(TOKEN_TENANT_ID));
        String apiKey = String.valueOf(claims.get(TOKEN_API_KEY));
        String datasetKey = String.valueOf(claims.get(TOKEN_DATASET_KEY));
        return UserWithPassword.get(userId, userName, displayName, email, password, isAdmin, tenantId, apiKey, datasetKey);
    }
}
